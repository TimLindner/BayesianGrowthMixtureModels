...
// vectors. thus, Y_obs, X, and X_squared are declared as one-dimensional arrays
// of row vectors.
...
data {
  ...
  // mean hyperparameters for Normal prior of quadratic trend components
  vector[C] mu_beta_2;
  
  // SD hyperparameters for Normal prior of quadratic trend components
  vector<lower=0>[C] sigma_beta_2;
...
model {
  ...
  // prior for beta_2
  beta_2 ~ normal(mu_beta_2, sigma_beta_2);  // vectorization over c
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    mu = beta_0[z[n]] + beta_1[z[n]] * X[n] + beta_2[z[n]] * X_squared[n];
    // vectorization over t
