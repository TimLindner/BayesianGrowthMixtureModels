...
### Future work
Define a strategy for setting the following hyperparameters so that label switching is prevented: $\sigma_{\beta_{0,c}}$, $\sigma_{\beta_{1,c}}$, and $\sigma_{\sigma_c}$. However, the hyperparameters are not allowed to be informative regarding classes.
