...
# Dataset6_Yobs.xlsx
# Dataset6_zsim.xlsx
# Model1ThreeClasses_Dataset6_Fit1.rds
```
...
* Initial values for $\beta_{0,c}$ and $\beta_{1,c}$: for every chain, the posterior means for $\beta_{0,c}$ and $\beta_{1,c}$ estimated via model 1 with identically distributed errors not only over individuals and time periods but also latent classes
...
z_sim <- data.frame(read_excel("SimulationStudyData/Dataset6_zsim.xlsx",
                               sheet = "Sheet 1"))
...
# estimated data ####
# load model fit
m_fit <- readRDS("SimulationStudyResults/Model1ThreeClasses_Dataset6_Fit1.rds")
...
# Convergence
```{r, include = FALSE}
# diagnostics: Rhat, Bulk ESS, and Tail ESS for all parameters
lambda_conv <- c(); beta_0_conv <- c(); beta_1_conv <- c(); sigma_conv <- c()
for (c in 1:C) {
  lambda_conv <- c(lambda_conv,paste("lambda[",c,"]", sep = ""))
  beta_0_conv <- c(beta_0_conv,paste("beta_0[",c,"]", sep = ""))
  beta_1_conv <- c(beta_1_conv,paste("beta_1[",c,"]", sep = ""))
  sigma_conv <- c(sigma_conv,paste("sigma[",c,"]", sep = ""))
}
params <- c(lambda_conv,beta_0_conv,beta_1_conv,sigma_conv)
...
for (c in 1:C) {
  ...
  plot(m_fit_data$lambda[,c],
       type = "l",
       col = "blueviolet",
       main = paste("Trace plot for mixture proportion c =", c),
...
# Posterior predictive checks via middle 90% credible intervals
```{r}
for (n in 1:N) {
