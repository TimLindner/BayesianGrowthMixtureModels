...
# folder:
# SimCase18_Yobslog_SimRun1.xlsx, SimCase18_zsim_SimRun1.xlsx
# SimCase18_Yobslog_SimRun2.xlsx, SimCase18_zsim_SimRun2.xlsx
# SimCase18_Yobslog_SimRun3.xlsx, SimCase18_zsim_SimRun3.xlsx
# SimCase18_Yobslog_SimRun4.xlsx, SimCase18_zsim_SimRun4.xlsx
# SimCase18_Yobslog_SimRun5.xlsx, SimCase18_zsim_SimRun5.xlsx
# and the corresponding file from the SimulationStudyResults/Model3 folder:
# e.g., for the first pair of files listed above,
# Model3TwoClasses_SimCase18_Yobs_SimRun1_Fit.rds
```
...
# Hyperparameters
* $\mu_{\beta_{0,c}} = \log\Bigg(\frac{1}{N} \displaystyle\sum_{n=1}^{N} y^{obs}_{n,t=1}\Bigg)$
...
* Initial values for $\beta_{0,c}$ and $\beta_{1,c}$: for every chain, the posterior means for $\beta_{0,c}$ and $\beta_{1,c}$ estimated via model 1 with identically distributed errors; using the simulation run 1 data; the posterior means for $\boldsymbol{\beta}_0$ are sorted in ascending order so that the initial values satisfy the labeling restriction; thus, the order of the posterior means for $\boldsymbol{\beta}_1$ is adjusted accordingly
...
Y_obs <- data.frame(read_excel(
    paste("SimulationStudyData/Model3/SimCase18_Yobs_SimRun",
          r,
          ".xlsx",
          sep = ""),
    sheet = "Sheet 1"))
...
z_sim <- data.frame(read_excel(
  paste(
    "SimulationStudyData/Model3/SimCase18_zsim_SimRun", r, ".xlsx", sep = ""),
  sheet = "Sheet 1"))
...
# load model fit
m_fit <- readRDS(paste(
  "SimulationStudyResults/Model3/Model3TwoClasses_SimCase18_Yobs_SimRun",
  r,
  "_Fit.rds",
  sep = ""))
