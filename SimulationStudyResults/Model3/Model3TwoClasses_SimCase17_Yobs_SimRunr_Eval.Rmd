...
* Initial values for $\beta_{0,c}$ and $\beta_{1,c}$: for every chain, the posterior means for $\beta_{0,c}$ and $\beta_{1,c}$ estimated via model 1 with identically distributed errors not only over individuals and time periods but also latent classes; the posterior means estimated for simulation run 1
