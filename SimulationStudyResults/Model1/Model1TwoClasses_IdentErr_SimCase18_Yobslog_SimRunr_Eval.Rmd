...
# and the corresponding model fit from the SimulationStudyResults/Model1 folder:
# e.g., for the first pair of datasets listed above,
# Model1TwoClasses_IdentErr_SimCase18_Yobslog_SimRun1_Fit.rds
```
...
* Initial values for $\beta_{0,c}$: for every chain, $\kappa_c$; where $\boldsymbol{\kappa}$ is a row vector of size $C$ storing the estimates of K-means clustering for $\boldsymbol{y}^{obs}_{t=1}$, with $\kappa_c \leq \kappa_{c+1}$
...
Y_obs_log <- data.frame(read_excel(
    paste("SimulationStudyData/Model3/SimCase18_Yobslog_SimRun",
          r,
          ".xlsx",
          sep = ""),
    sheet = "Sheet 1"))
...
z_sim <- data.frame(read_excel(
  paste("SimulationStudyData/Model3/SimCase18_zsim_SimRun",
        r,
        ".xlsx",
        sep = ""),
  sheet = "Sheet 1"))
...
paste_part_1 <- "SimulationStudyResults/Model1/"
paste_part_2 <- "Model1TwoClasses_IdentErr_SimCase18_Yobslog_SimRun"
